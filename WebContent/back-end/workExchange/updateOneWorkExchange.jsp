					...
					<div class="row">
						...
						<Form method="post"
							...
							<table>
								...
								<tr>
									<th>創建員工:</th>
									<td>${employeeSvc.getOneEmp(workExchangeVO.empID).empName}
										<input type="hidden" name="empID"
										value="${workExchangeVO.empID}">
									</td>
								</tr>
								<tr>
									<th>會員姓名:</th>
									<td>${memberSvc.getOneMem(workExchangeVO.memID).memName}<input
										type="hidden" name="memID" value="${workExchangeVO.memID}"></td>
								</tr>
								<tr>
									<th>房型名稱:</th>
									<td>${roomTypeSvc.getOneRoomType(workExchangeVO.rtID).rtName}
										<input type="hidden" name="rtID"
										value="${workExchangeVO.rtID}">
									</td>
								</tr>
								<tr>
									<th>需求名稱:</th>
									<td>${workExchangeVO.weName}<input type="hidden"
										name="weName" value="${workExchangeVO.weName}"></td>
								</tr>
								<tr>
									<th>需求內容:</th>
									<td>${workExchangeVO.weContent}<input type="hidden"
										name="weContent" value="${workExchangeVO.weContent}"></td>
								</tr>
								<tr>
									<th>需求照片:</th>
									<td><input type="file" name="wePic"> <%-- 						<input type="hidden" name="wePic" value="${workExchangeVO.wePic}"> --%>
									</td>
								</tr>
								<tr>
									<th>需求影片:</th>
									<td><input type="file" name="weVideo"> <%-- 						<input type="hidden" name="weVideo" value="${workExchangeVO.weVideo}"> --%>
									</td>
								</tr>
								<tr>
									<th>開始時間:</th>
									<td>${workExchangeVO.weStart}<input type="hidden"
										name="weStart" value="${workExchangeVO.weStart}"></td>
								</tr>
								<tr>
									<th>結束時間:</th>
									<td>${workExchangeVO.weEnd}<input type="hidden"
										name="weEnd" value="${workExchangeVO.weEnd}"></td>
								</tr>
							</table>
							<input type="hidden" name="weID" value="${workExchangeVO.weID}">
							<input type="submit" class="btn-success form-control"
								value="確認修改"> <input type="hidden" name="action"
								value="confirm_Modify">
						</Form>
					</div>
				</div>
