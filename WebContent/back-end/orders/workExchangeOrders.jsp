...
<%
	...
	List<OrdersVO> list = ordSvc.findByordType2("B01");
					...
					<h2>打工換宿訂單列表</h2>
						...
						<table class="table table-bordered table-striped table-hover">
							...
							<c:forEach var="ordVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
								...
								<tr>
									<td><a href="<%=request.getContextPath()%>/orders/orders.do?ordID=${ordVO.ordID}&action=getWE_OrderDetail">${ordVO.ordID}</a></td>
									<td>${ordVO.memID}</td>
									<td>${ordVO.braID}</td>
