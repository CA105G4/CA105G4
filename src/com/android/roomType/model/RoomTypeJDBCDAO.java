...
public class RoomTypeJDBCDAO implements RoomTypeDAO_interface{
	...
	private static final String GET_ALL_STMT = "SELECT rtId, braId, rtName, rtIntro, holidayprice FROM roomType WHERE braId != 'B00'";
...
//	private static final String GET_BY_BRANAME_STMT = "SELECT roomType.rtId, roomType.braId, roomType.rtName, roomType.rtIntro, roomType.holidayPrice, roomType.balance FROM branch" + 
//			"INNER JOIN roomType on branch.braId = roomType.braId WHERE branch.braName = ?";
	private static final String GET_BY_BRANAME_STMT = "SELECT * FROM branch INNER JOIN roomType on branch.braId = roomType.braId WHERE branch.braName = ?";
...
//	}
	...
	@Override
	public List<RoomTypeBra> findByBraName(String braName){
		List<RoomTypeBra> list = new ArrayList<RoomTypeBra>();
		RoomTypeBra roomTypeVO = null;
		...
		try {
			Class.forName(driver);
			con = DriverManager.getConnection(url, userid, passwd);
			pstmt = con.prepareStatement(GET_BY_BRANAME_STMT);
			
			pstmt.setString(1, braName);
			
			rs = pstmt.executeQuery();
			
			while(rs.next()) {
				roomTypeVO = new RoomTypeBra();
				roomTypeVO.setRtID(rs.getString("RTID"));
				roomTypeVO.setBraID(rs.getString("BRAID"));
				roomTypeVO.setRtName(rs.getString("RTNAME"));
				roomTypeVO.setRtIntro(rs.getString("RTINTRO"));
...
//		System.out.println("---------------------------------------------");
		...
		List<RoomTypeBra> list2 = dao.findByBraName("福翔");
		
		for(RoomTypeBra rt : list2) {
			System.out.println(rt.getRtID());
			System.out.println(rt.getBraID());
			System.out.println(rt.getRtName());
			System.out.println(rt.getRtIntro());
